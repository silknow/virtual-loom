Shader "VirtualLoom/Yarn" {
	Properties {
		...
		// Metallic band
		_MetallicBand("Metallic Band", Range(0.02, 1.0)) = 0.0
	...
	//	#endif
		...
		void surf (Input IN, inout SurfaceOutputLuxAnisoMetallic o) {
			...
			half2 uv= half2(IN.uv_MainTex.x*_Threads+IN.uv_MainTex.y*_Twist+random(IN.uv_MainTex)*_NoisePower,IN.uv_MainTex.y*_Repeat*_Threads);
			half2 uvMetal= half2(IN.uv_MainTex.x*_Threads+IN.uv_MainTex.y*_MetalTwist,IN.uv_MainTex.y*_Repeat*_Threads);
			//uv = IN.uv_MainTex;
			fixed4 c = lerp(tex2D (_MainTex, uvMetal) * _MetalColor,tex2D (_MainTex, uv) * _Color,step(_MetallicBand,uvMetal.x*_Repeat-round(uvMetal.x*_Repeat)+0.5));
			o.Alpha = c.a;
		...
		//	Regular Alpha Testing
			...
			o.Albedo = c.rgb * IN.color.g;
				...
				half m = metallicGloss.r * _Metallic; // That is how the standard shaders handles it...
				o.Metallic = lerp(1,m,step(_MetallicBand,uvMetal.x*_Repeat-round(uvMetal.x*_Repeat)+0.5));
				//o.Metallic = uv.x-round(uv.x);				
			...
			half4 n = tex2D(_BumpMap, uv);
			n = lerp(half4(0.5,0.5,0.5,0.5),n,step(_MetallicBand,uvMetal.x*_Repeat-round(uvMetal.x*_Repeat)+0.5));
			o.Normal = UnpackNormal( n ) * float3(1.0, 1.0, IN.FacingSign);
		...
		//	Lux: Anisotropic features
			...
			o.TangentDir = lerp( _BaseTangentDir, UnpackNormal( tex2D(_TangentDir, uv)), _TangentDirStrength) * float3(IN.FacingSign, 1, 1);
